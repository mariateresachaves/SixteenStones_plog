...
A cor das peças dos jogadores são representadas pelos carateres \textit{X} para as peças azuis e \textit{O} para as peças vermelhas, sendo que o carater \textit{\_} será usado para representar casas vazias.

Os predicados utilizados para construir e visualizar o tabuleiro foram os seguintes:

\begin{itemize}
	\item \textit{make\_line/2} - Gera uma linha do tabuleiro
	\item \textit{make\_board/2} - Gera um tabuleiro de jogo com o tamanha passado
	\item \textit{print\_line/1} - Predicado para imprimir uma linha do tabuleiro
	\item \textit{print\_board/1} - Predicado que imprime o board, passado sob a forma de lista de listas
\end{itemize}
